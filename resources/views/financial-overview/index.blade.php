<x-app-layout>
    ...
    <style>
        ...
        .progress-budget { height: 10px; border-radius: 10px; overflow: hidden; background: var(--bs-tertiary-bg); }
        .activity-item {
        ...
        .payment-badge { font-size: 0.75rem; padding: 0.3rem 0.6rem; }
    ...
    <!-- Money Available -->
    <div class="alert alert-gold d-flex align-items-center gap-3" role="alert"
         style="background: linear-gradient(135deg, rgba(198, 169, 92, 0.1), rgba(198, 169, 92, 0.05));
                ...
                <div class="card-body p-4">
                    ...
                    <div class="mb-3">
                        <div class="d-flex justify-content-between mb-1">
                            <small class="text-muted">Total: Rp {{ number_format($data['budget']['total_amount'], 0, ',', '.') }}</small>
                            <small class="text-muted">Terpakai: {{ $data['budget']['usage_percent'] }}%</small>
                        </div>
                        <div class="progress-budget">
                            <div class="track" style="width: {{ min(100, $data['budget']['usage_percent']) }}%; background: var(--adminuiux-theme-1);"></div>
                        </div>
                    </div>
                    ...
                    <div class="row g-2 text-center mb-2">
                        ...
                        <div class="col-4">
                            <div class="fw-bold text-success">{{ number_format($data['budget']['total_remaining'], 0, ',', '.') }}</div>
                            <small class="text-muted">Tersisa</small>
                        </div>
                        <div class="col-4">
                            <div class="fw-bold {{ $data['budget']['is_over_budget'] ? 'text-danger' : 'text-success' }}">
                    ...
                    <div class="mt-3">
                        ...
                        <div class="d-flex align-items-center gap-2 mb-1">
                            <span class="badge rounded-circle" style="background: {{ $cat['colour'] }}; width: 14px; height: 14px;"></span>
                            <span class="small">{{ $cat['name'] }}</span>
                            <span class="ms-auto small text-muted">
                                {{ number_format($cat['spent'], 0, ',', '.') }} / {{ number_format($cat['allocated'], 0, ',', '.') }}
                            </span>
                        </div>
                        @endforeach
                    </div>
        ...
        <div class="col-lg-6">
            <div class="card stat-card-custom shadow-sm h-100">
                <div class="card-body p-4">
                    ...
                    <div class="mb-3">
                        <div class="d-flex justify-content-between mb-1">
                            <small class="text-muted">Total Target: Rp {{ number_format($data['savings']['total_target'], 0, ',', '.') }}</small>
                            <small class="text-muted">Progres: {{ $data['savings']['progress_percent'] }}%</small>
                        </div>
                        <div class="progress-budget">
                            <div class="track" style="width: {{ min(100, $data['savings']['progress_percent']) }}%; background: #28a745;"></div>
                        </div>
                    </div>
                    ...
                    <div class="alert alert-info d-flex align-items-center gap-2 py-2 mb-0 rounded" style="background: rgba(23, 162, 184, 0.1);">
                        <i class="bi bi-automation"></i>
                        <div class="small">
                            <strong>Setoran otomatis berikutnya:</strong> Rp {{ number_format($data['savings']['next_auto_contribution']['amount'], 0, ',', '.') }}
                            pada {{ \Carbon\Carbon::parse($data['savings']['next_auto_contribution']['date'])->format('d M Y') }}
                    ...
                    <div class="mt-3">
                        ...
                        <div class="d-flex align-items-center justify-content-between mb-1">
                            ...
                            <span class="small text-muted">{{ number_format($goal['saved'], 0, ',', '.') }}/{{ number_format($goal['target'], 0, ',', '.') }}</span>
                        </div>
                        <div class="progress-budget mb-2" style="height: 6px;">
                            <div class="track" style="width: {{ min(100, $goal['progress_percent']) }}%; background: {{ $goal['colour'] }};"></div>
                        </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="card stat-card-custom shadow-sm h-100">
                <div class="card-body p-4">
                    <h6 class="fw-bold mb-3 d-flex align-items-center justify-content-between">
                        ...
                        <a href="{{ route('budget.payment.index') }}" class="btn btn-sm btn-outline-secondary">Lihat Semua</a>
                    </h6>
                    ...
                    <div class="d-flex flex-column gap-3">
                        ...
                        <div class="d-flex align-items-center justify-content-between p-3 border rounded" style="border-radius: 12px;">
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <small class="text-muted">{{ \Carbon\Carbon::parse($p['scheduled_date'])->format('d M Y') }}</small>
                                </div>
                            </div>
                            <div class="text-end">
                                <div class="fw-bold">Rp {{ number_format($p['amount'], 0, ',', '.') }}</div>
                                <span class="badge payment-badge bg-warning-subtle text-warning border border-warning-subtle">
                ...
                <div class="card-body p-4">
                    ...
                    <div class="activity-feed">
                        ...
                        <div class="activity-item d-flex align-items-center gap-3 py-2">
                            <div class="stat-icon-box {{ $act['type'] == 'contribution' ? 'bg-success-subtle text-success' : 'bg-primary-subtle text-primary' }} flex-shrink-0">
                                <i class="bi {{ $act['type'] == 'contribution' ? 'bi-plus-circle' : 'bi-receipt' }}"></i>
                            </div>
                            ...
                            <div class="text-end">
                                <div class="fw-bold {{ $act['type'] == 'contribution' ? 'text-success' : 'text-danger' }}">
                                    {{ $act['type'] == 'contribution' ? '+' : '-' }}Rp {{ number_format($act['amount'], 0, ',', '.') }}
                                </div>
