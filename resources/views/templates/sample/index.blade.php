...
<body class="bg-black text-[#D4AF37] font-sans flex justify-center">
    ...
    <div class="w-full max-w-[420px] min-h-screen bg-[#0F0F0F] relative overflow-hidden shadow-2xl border-x border-primary/20">
        ...
        <header class="relative h-screen flex items-center justify-center text-center px-8">
            <div id="preview-hero-bg" class="absolute inset-0 bg-cover bg-center opacity-30" style="background-image: url('{{ asset('storage/' . $invitation->gallery_cover) }}');"></div>
    ...
    <script>
        ...
        // Music
        const bgM = document.getElementById('bgMusic'), mB = document.getElementById('musicBtn');
        let hasI = false;
        ...
        function play() { bgM.play().catch(() => {}); mB.innerHTML = '⏸'; }
        function pause() { bgM.pause(); mB.innerHTML = '▶'; }
        window.addEventListener('scroll', () => { if (!hasI) { play(); hasI = true; } }, { once: true });
        mB.onclick = () => mB.innerHTML === '▶' ? play() : pause();
