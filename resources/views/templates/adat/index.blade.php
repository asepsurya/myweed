...
<head>
    ...
    <meta property="og:title"
        content="{{ $invitation->groom_nickname }} & {{ $invitation->bride_nickname }} Wedding Invitation">
    <meta property="og:description"
        content="You are invited to the wedding of {{ $invitation->groom_nickname }} & {{ $invitation->bride_nickname }}.">
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Cinzel:wght@400;600&family=Cormorant+Garamond:ital,wght@0,400;0,600;1,400&family=Lato:wght@300;400&display=swap"
        rel="stylesheet">
    ...
    <style>
        ...
        * {
        ...
        .hero {
            height: 100vh;
            background: linear-gradient(rgba(74, 16, 24, 0.85), rgba(74, 16, 24, 0.6)), url('{{ storage_url($invitation->gallery_cover) }}');
        ...
        .video-play-overlay {
            ...
            align-items: center;
            justify-content: center;
            background: rgba(74, 16, 24, 0.4);
            cursor: pointer;
            transition: background 0.3s;
        }

        .video-play-overlay:hover {
            background: rgba(74, 16, 24, 0.2);
        }
...
<body>
    ...
    <div class="mobile-container">
        ...
        <header id="preview-hero-bg" class="hero">
            ...
            <p class="fade-in" style="font-family: 'Cinzel', serif; font-size: 0.9rem; letter-spacing: 3px;">The Wedding
                Of</p>
            ...
            <div class="hero-sub fade-in">
                ...
                <span
                    style="font-weight: 700; font-size: 1.3rem; display: inline-block; margin-top: 5px; color: white;">
                    {{ request('penerima') ?? 'Tamu Undangan' }}
                </span>
            </div>
        ...
        <div class="content-wrapper">
                ...
                <div class="couple-wrapper fade-in">
                    <div class="couple-card">
                        <div class="photo-frame-javanese">
                            <img loading="lazy" id="preview-foto-pria" src="{{ storage_url($invitation->foto_pria) }}"
                                alt="{{ $invitation->groom_name }}" class="couple-img">
                        </div>
                        <h3 class="couple-name">{{ $invitation->groom_name }}</h3>
                        <p class="parent-name">
                            {{ $invitation->groom_child_order_text ? 'Putra ' . $invitation->groom_child_order_text . ' dari' : 'Putra dari' }}
                            Bpk. {{ $invitation->groom_father_name }}<br>& Ibu {{ $invitation->groom_mother_name }}
                        </p>
                    </div>
                    ...
                    <div class="couple-card">
                        <div class="photo-frame-javanese">
                            <img loading="lazy" id="preview-foto-wanita"
                                src="{{ storage_url($invitation->foto_wanita) }}" alt="{{ $invitation->bride_name }}"
                                class="couple-img">
                        </div>
                        <h3 class="couple-name">{{ $invitation->bride_name }}</h3>
                        <p class="parent-name">
                            {{ $invitation->bride_child_order_text ? 'Putri ' . $invitation->bride_child_order_text . ' dari' : 'Putri dari' }}
                            Bpk. {{ $invitation->bride_father_name }}<br>& Ibu {{ $invitation->bride_mother_name }}
                        </p>
                    </div>
                </div>
            </section>
                ...
                <div class="fade-in">
                    <div class="event-card">
                        ...
                        <p style="font-size: 1.1rem; color: #555;">
                            {{ $invitation->akad_location }}<br>{{ $invitation->akad_address }}
                        </p>
                    ...
                    <div class="event-card">
                        ...
                        <p style="font-size: 1.1rem; color: #555;">
                            {{ $invitation->resepsi_location }}<br>{{ $invitation->resepsi_address }}
                        </p>
                ...
                <section class="section-padding" style="background:#f7f7f7;">
                    ...
                    @foreach($invitation->gifts as $gift)
                        <div class="gift-card fade-in">
                            ...
                            <p style="font-size: 1.4rem; font-weight: bold; margin: 10px 0; font-family: 'Cinzel', serif;">
                                {{ $gift->number }}
                            </p>
                            <p>A/N: {{ $gift->name }}</p>
                            <button onclick="copyToClipboard('{{ $gift->number }}')" class="btn-map"
                                style="padding: 8px 20px; font-size: 0.8rem; margin-top: 15px;">Salin No. Rekening</button>
                        </div>
                    @endforeach
                </section>
            @endif
            ...
            @if($invitation->enable_rsvp == 1)
                <section class="section-padding" style="background: var(--bg-color);">
                    ...
                    <div class="rsvp-form fade-in">
                        <form id="rsvpForm">
                            @csrf
                            <input type="text" name="name" id="rsvpName" class="form-control" placeholder="Nama Anda"
                                required>
                            ...
                            <div class="emoji-picker"
                                style="margin-bottom: 15px; display: flex; gap: 15px; justify-content: center;">
                            ...
                            <textarea name="message" id="rsvpMessage" class="form-control" rows="3"
                                placeholder="Tulis ucapan selamat..." required></textarea>
                    ...
                    <div class="fade-in" style="margin-top: 40px;">
                        <div id="rsvpList" class="rsvp-list">
                            <div class="text-center" id="emptyRsvp" style="color: #888; font-style: italic;">Belum ada
                                ucapan. Jadilah yang pertama!</div>
                        </div>
                    </div>
                </section>
            @endif
            ...
            <footer>
                <div class="fade-in">
                    <svg class="hero-ornament"
                        style="margin-bottom: 20px; stroke: var(--accent-color); fill: none; stroke-width: 1.5; width: 80px;"
                        viewBox="0 0 100 50">
                        <path
                            d="M50 50 C 30 30, 10 30, 0 50 M50 50 C 70 30, 90 30, 100 50 M50 0 V 50 M40 20 Q 50 10 60 20" />
                    </svg>
                    <h2 style="font-size: 2rem; color: var(--accent-color);">
                        {{ $invitation->groom_nickname }} & {{ $invitation->bride_nickname }}
                    </h2>
                    <p
                        style="font-family: 'Cormorant Garamond', serif; font-size: 1.1rem; margin-top: 15px; opacity: 0.9;">
                        Matur nuwun ingkang agung ngaturaken pangestu lan kehadiripun.
                    </p>
                    <br>
                    <small style="opacity: 0.5; font-family: 'Lato', sans-serif;">&copy; {{ date('Y') }} Undangan
                        Pernikahan Adat Jawa</small>
                </div>
            </footer>
        </div>
    ...
    @if(($invitation->enable_love_story ?? true) && !empty($invitation->love_story))
        ...
        @if(!empty($loveStories[0]['title']) || !empty($loveStories[0]['story']))
            <section class="py-16 px-6 text-center fade-in" id="love-story">
                <h2 class="text-3xl font-serif text-primary mb-6">Kisah Kami</h2>
                <div class="max-w-2xl mx-auto">
                    @foreach($loveStories as $index => $story)
                        <div
                            class="mb-8 {{ $index < count($loveStories) - 1 ? 'pb-8 border-b border-dashed border-gray-200' : '' }}">
                            <h3 class="text-lg font-semibold text-gray-700 mb-2">{{ $story['title'] ?? '' }}</h3>
                            <p class="text-gray-600 leading-relaxed">{{ $story['story'] ?? '' }}</p>
                            @if(!empty($story['photo']))
                                <img src="{{ storage_url($story['photo']) }}" alt="{{ $story['title'] ?? 'Story Photo' }}"
                                    loading="lazy" class="mt-3 max-h-[200px] object-cover rounded-lg mx-auto">
                            @endif
                        </div>
                    @endforeach
                </div>
    ...
    <!-- Video Pernikahan Section (Dirapihkan) -->
    @if(($invitation->enable_video ?? true) && !empty($invitation->video_link))
        ...
        <section class="section-padding text-center fade-in" id="video" style="background-color: var(--white);">
            <div class="mb-5">
                <h2 style="font-size: 2rem;">Video Pernikahan</h2>
                <div class="batik-line" style="width: 50px; margin: 15px auto;"></div>
            </div>

            @if($youtubeVideoId)
                <div class="photo-frame-javanese fade-in"
                    style="display: block; width: 100%; max-width: 350px; margin: 0 auto;">
                    <div class="video-wrapper" data-fancybox="video"
                        data-src="https://www.youtube.com/embed/{{ $youtubeVideoId }}?enablejsapi=1&autoplay=1&loop=1&playlist={{ $youtubeVideoId }}&controls=1&modestbranding=1&rel=0">
                        <img src="https://img.youtube.com/vi/{{ $youtubeVideoId }}/hqdefault.jpg" alt="Video Pernikahan">
                        <div class="video-play-overlay">
                            <i class="ti ti-player-play-filled video-play-icon"></i>
                        </div>
                    </div>
                </div>
            @else
