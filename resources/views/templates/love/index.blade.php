...
<body class="bg-gray-300 flex justify-center font-sans">
    ...
    <button id="musicToggle"
        class="fixed bottom-6 right-6 bg-primary text-white p-3 rounded-full shadow-lg flex items-center justify-center z-50 hover:bg-primary/90 transition">
        <!-- Icon default: play -->
    ...
    <div class="w-full max-w-[420px] min-h-screen bg-[#fdfaf6] relative overflow-hidden shadow-2xl">
        ...
        <section id="cover" class="relative h-screen flex items-center justify-center text-center">
            <div
                class="absolute inset-0 bg-[url('{{ asset('tempelate/love/assets/images/slider/h3.jpg') }}')] bg-cover bg-center lazy-bg">
            </div>
            <div class="absolute inset-0 bg-black/50"></div>

            <div class="relative z-10 text-white px-6 space-y-4">
                <p class="text-sm tracking-widest uppercase">Undangan Pernikahan</p>
                <h1 class="font-serif text-5xl font-bold">{{ $invitation->bride_name }} & {{ $invitation->groom_name }}
                </h1>
                <p class="text-sm">
                    Kepada Yth<br>
        ...
        <section id="content" class="space-y-14 py-12 ">
            ...
            @foreach ($invitation->template->sections as $section)
                @includeIf('templates.love.sections.' . $section)
            @endforeach
