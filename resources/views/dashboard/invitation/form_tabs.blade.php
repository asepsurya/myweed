...
@media (max-width: 991px) {
    ...
    .builder-wrapper {
        ...
        top: 0; 
        left: 0; 
        z-index: 1040; /* Ubah dari 9999 menjadi 1040 agar modal bisa muncul di atasnya */
    ...
    /* Pastikan modal crop memiliki z-index lebih tinggi dari wrapper */
    #cropModal {
    ...
    .music-source-tabs label {
        ...
        cursor: pointer; transition: all 0.3s ease; color: var(--bs-secondary-color);
        font-weight: 600; font-size: 0.85rem; border: none; background: transparent;
    ...
    .audio-player-wrapper audio { width: 100%; height: 38px; }
            ...
            <div class="row g-3">
                ...
                <div class="col-12">
                    ...
                    <div id="previewContainerGroom" class="mt-3 {{ ($inv && $inv->foto_pria) ? '' : 'd-none' }} text-center position-relative">
                        <img id="previewGroom" src="{{ ($inv && $inv->foto_pria) ? asset('storage/' . $inv->foto_pria) : '' }}" class="img-fluid rounded border shadow-sm" style="max-height: 200px; object-fit: cover;">
        ...
        <div class="card-body p-0">
            ...
            <div class="row g-3" id="templateGallery">
                @foreach ($templates as $template)
                    ...
                    <div class="col-6 template-selector-item" data-category="{{ $template->category->name ?? 'modern' }}" data-name="{{ strtolower($template->name) }}" data-color="{{ $template->primary_color ?? '#0d9488' }}" data-template-id="{{ $template->id }}">
                        <div class="card template-card-selector h-100 {{ $isLocked ? 'locked opacity-75' : '' }} {{ ($inv && $inv->template_id == $template->id) ? 'selected' : '' }}" onclick="{{ $isLocked ? 'showPremiumAlert()' : 'selectTemplate(this, ' . $template->id . ')' }}">
            ...
            <div class="mb-4">
                ...
                <div id="previewContainerCover" class="mt-3 {{ ($inv && $inv->gallery_cover) ? '' : 'd-none' }} text-center position-relative">
                    <img id="previewCover" src="{{ ($inv && $inv->gallery_cover) ? asset('storage/' . $inv->gallery_cover) : '' }}" class="img-fluid rounded border shadow-sm w-100" style="max-height: 250px; object-fit: cover;">
                    ...
                    @if($inv && $inv->galleries)
                        @foreach($inv->galleries as $image)
                            <div class="position-relative border rounded overflow-hidden" style="width: 80px; height: 80px;">
                                ...
                                <button type="button" class="btn btn-danger btn-sm position-absolute top-0 end-0 p-0" style="width:20px;height:20px; line-height: 1;" onclick="deleteGallery({{ $image->id }})">&times;</button>
                            </div>
                        @endforeach
                    @endif
                </div>
                ...
                <div class="music-content-box">
                    ...
                    <div id="source-upload" class="music-source-div {{ $musicSource != 'upload' ? 'd-none' : '' }}">
                        ...
                        <div class="upload-box-custom" onclick="document.getElementById('custom_music_input').click()">
                            ...
                            <small class="text-muted" id="upload_file_name">@if(!empty($inv->custom_music)) {{ basename($inv->custom_music) }} @else Belum ada file terpilih @endif</small>
                        </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-12">
                    ...
                    <div id="previewContainerBride" class="mt-3 {{ ($inv && $inv->foto_wanita) ? '' : 'd-none' }} text-center position-relative">
                        <img id="previewBride" src="{{ ($inv && $inv->foto_wanita) ? asset('storage/' . $inv->foto_wanita) : '' }}" class="img-fluid rounded border shadow-sm" style="max-height: 200px; object-fit: cover;">
                            ...
                            <div class="mb-2">
                                ...
                                @if(!empty($story['photo']))
                                    <div class="position-relative d-inline-block">
                                        ...
                                        <button type="button" class="btn btn-danger btn-sm position-absolute top-0 end-0 m-1" style="width:20px;height:20px;line-height:1;padding:0;" onclick="this.closest('.position-relative').remove(); this.closest('.love-story-item').querySelector('input[name=\'story_photo[]\']').value = '';">&times;</button>
                                    </div>
                                @endif
