<x-app-layout>
    ...
    @php
        // Hitung statistik header di atas agar HTML lebih bersih
        $totalTargetAmount = $goals->sum('target_amount');
        $totalSavedAmount = $goals->sum('contributions_sum_amount');
        $overallProgress = $totalTargetAmount > 0 ? round(($totalSavedAmount / $totalTargetAmount) * 100, 1) : 0;
                ...
                <div class="card stat-card-custom shadow-sm h-100">
                    <div class="card-body d-flex align-items-center gap-3 p-3">
                        ...
                        <div class="overflow-hidden">
                            <div class="fw-bold fs-5 text-truncate">{{ $goals->count() }}</div>
                            <div class="text-muted small text-truncate">Total Target</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="stat-slide">
                <div class="card stat-card-custom shadow-sm h-100">
                    <div class="card-body d-flex align-items-center gap-3 p-3">
                        ...
                        <div class="overflow-hidden">
                            <div class="fw-bold fs-5 text-truncate">Rp {{ number_format($totalSavedAmount, 0, ',', '.') }}</div>
                            <div class="text-muted small text-truncate">Total Tertabung</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="stat-slide">
                <div class="card stat-card-custom shadow-sm h-100">
                    <div class="card-body d-flex align-items-center gap-3 p-3">
                        ...
                        <div class="overflow-hidden">
                            <div class="fw-bold fs-5 text-truncate">{{ $overallProgress }}%</div>
                            <div class="text-muted small text-truncate">Progres</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="stat-slide">
                <div class="card stat-card-custom shadow-sm h-100">
                    <div class="card-body d-flex align-items-center gap-3 p-3">
                        ...
                        <div class="overflow-hidden">
                            <div class="fw-bold fs-5 text-truncate">{{ $overdueCount }}</div>
                            <div class="text-muted small text-truncate">Lewat Deadline</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <table class="table table-custom table-hover align-middle mb-0">
                <thead>
                    <tr>
                        ...
                        <th class="text-end">Target Jumlah</th>
                        <th class="d-none d-sm-table-cell text-end">Terkumpul</th>
                        <th class="d-none d-md-table-cell">Deadline</th>
                        <th class="d-none d-lg-table-cell text-center">Auto</th>
                        <th class="d-none d-md-table-cell">Progres</th>
                    ...
                    <tr>
                        <td class="ps-4">
                            <div class="d-flex align-items-center gap-2">
                                <span class="badge rounded-circle flex-shrink-0" style="background: {{ $goal->colour }}; width: 12px; height: 12px;"></span>
                        ...
                        <td class="text-end fw-semibold">Rp {{ number_format($goal->target_amount, 0, ',', '.') }}</td>
                        <td class="d-none d-sm-table-cell text-end">Rp {{ number_format($goal->contributions_sum_amount ?? 0, 0, ',', '.') }}</td>
                        ...
                        <td class="text-end pe-4">
                            <div class="d-flex justify-content-end gap-1">
                                <form action="{{ route('savings.goal.toggle', $goal) }}" method="POST" class="d-inline">
                                    @csrf
                                    <button type="submit" class="btn btn-sm {{ $goal->is_active ? 'btn-outline-warning' : 'btn-outline-success' }} btn-action" title="{{ $goal->is_active ? 'Non-aktifkan' : 'Aktifkan' }}">
                                        <i class="bi {{ $goal->is_active ? 'bi-eye' : 'bi-eye-slash' }}"></i>
                                    </button>
                                </form>
                                <a href="{{ route('savings.goal.edit', $goal) }}" class="btn btn-sm btn-outline-secondary btn-action" title="Edit">
                                    <i class="bi bi-pencil"></i>
                                </a>
                                <button type="button" class="btn btn-sm btn-outline-danger btn-action" onclick="confirmDeleteGoal({{ $goal->id }})" title="Hapus">
